...
if len(sys.argv) > 2:
	...
	if sys.argv[i] == "-v" or sys.argv[i] == "--verbose":
	    verbose = True
        if i < len(sys.argv)-1:
            if sys.argv[i] in ["--since", "--since-id"]:
                since_id = int(sys.argv[i+1])
            if sys.argv[i] in ["--max", "--max-id"]:
...
if opt == 4 or opt == 5:
	...
	#cw = csv.writer(f, delimiter="\t", quoting=csv.QUOTE_MINIMAL, quotechar='"', escapechar="\\", lineterminator="\n")
        cw = csv.writer(f)
	cw.writerow(["id", "created_at", "text", "in_reply_to_user_id", "in_reply_to_screen_name", "in_reply_to_status_id", "screen_name", "source", "user_id"])
    elif todb:
    	if verbose:
    	    print "to DB"
    #missed_once = False
	...
	if verbose:
	    print url
	try:
            if since_id == 0 and max_id == 0:
                resp, content = client.request(url + "&page=" + str(page), "GET")
            elif since_id != 0 and max_id != 0:
                resp, content = client.request(url + "&since_id=" + str(since_id) + "&max_id=" + str(max_id), "GET")
            elif since_id != 0:
                resp, content = client.request(url + "&since_id=" + str(since_id), "GET")
            elif max_id != 0:
                resp, content = client.request(url + "&max_id=" + str(max_id), "GET")
	    missed = False
	except:
	    missed = True
	if verbose:
	    ...
	    print content
	tries = 0
	while missed or (resp['status'] != '200' and tries < MAXIMUM_TRIES):
	    tries += 1
	...
	if resp['status'] == '200':
	    js = simplejson.loads(content)
	    if "x-rate-limit-remaining" in resp and int(resp['x-rate-limit-remaining']) <= 20:
		time.sleep(400)
	...
	for j in range(len(js)):
	    ...
	    if tocsv:
		r = list()
		r = [l["id"], l["created_at"], l["text"].encode("utf8"), l["in_reply_to_user_id"], l["in_reply_to_screen_name"], l["in_reply_to_status_id"], l["source"]]
		if opt == 4:
		    r.extend(["",user_id])
		elif opt == 5:
